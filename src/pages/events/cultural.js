...
export const getStaticProps = async () => {
  ...
  query = `*[_type == 'events' && type == 'tech']{name, short, date, slug{current},meta{image{asset->{url}}},image { asset -> {url}}, type}`;
  res = await fetch(url + encodeURIComponent(query));
  const techData = await res.json();

  return {
    props: { data: data.result, techData: techData.result },
  };
};

const Events = ({ data, techData }) => {
    ...
    <>
      ...
      <main className={styles.eventMain}>
        ...
        <div className={styles.mainMask}>
          <h1>Events</h1>
          <Container fluid className={styles.cardContainerTech}>
            <Row>
              <Col>
                <h1>Tech Events</h1>
              </Col>
